...
```
package import var const func type struct if else for switch return break continue true false nil interface destructor comptime comptime_error
```
...
Mutability depends **only** on method receiver type:

- Pointer receiver → mutates underlying value e.g:
  ```ori
  func (self *File) Write(p []byte) int { ... }
  ```
- Value receiver → operates on a copy e.g:
  ```ori
  func (self File) Describe() string { ... }
  ```

Interfaces introduce **no new mutability rules**.
...
func (self *File) Write(p []byte) string { ... }
